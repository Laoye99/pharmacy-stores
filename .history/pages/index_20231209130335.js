...
export default function Home(props) {
  ...
  const [export async function getStaticProps() {
  try {
    // Fetch your data
    const pharmacyStores = await fetchData();

    // Ensure that the data is properly defined
    if (!pharmacyStores || !Array.isArray(pharmacyStores)) {
      throw new Error("Invalid data structure");
    }

    // Clean up the data, replacing undefined with null or omitting the property
    const cleanedPharmacyStores = pharmacyStores.map(store => ({
      ...store,
      locality: store.locality || null, // Replace undefined with null
      // Other properties...
    }));

    return {
      props: {
        pharmacyStores: cleanedPharmacyStores,
      },
    };
  } catch (error) {
