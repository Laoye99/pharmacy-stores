...
export async function getStaticProps({ params }) {
    try {
      ...
      // Find the pharmacy store by ID
      const findPharmacyStoreById = pharmacyStores.find(
        (pharmacyStore) => pharmacyStore.id.toString() === params.id
      );
  
      // Check if the pharmacy store is found, replace undefined properties with null
      const sanitizedPharmacyStore = findPharmacyStoreById
        ? {
            ...findPharmacyStoreById,
            locality: findPharmacyStoreById.locality || null,
      ...
      return {
        props: {
          pharmacyStore: sanitizedPharmacyStore,
        },
      };
    } catch (error) {
