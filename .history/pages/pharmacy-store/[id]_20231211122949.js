...
const PharmacyStore = (initialProps) => {
    ...
    console.log("props.coffeeStore", initialProps.pharmacyStore);

    const id =router.query.id;

    const [pharmacyStore, setpharmacyStore] = useState(init pharmacyStore);
    ...
    useEffect(() => {
        if(isEmpty(initialProps.pharmacyStore)) {
            if (pharmacyStores.length>0){
                ...
                setpharmacyStore(findPharmacyStoreById);
            }
        }
    }, [id]);

    const {name, formatted_address, locality, imgUrl } = initialProps.pharmacyStore;
